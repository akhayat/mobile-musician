...
- (void)queueNote:(int)midiNoteNumber gain:(float)gain;
{
	...
	if (n->bufferIndex != -1)
	{
		...
		if (sourceIndex != -1)
		{
			...
			Buffer* buffer = buffers + n->bufferIndex;
			...
			alSourcef(source->sourceId, AL_PITCH, n->pitch/buffer->pitch);
			...
			float sourcePos[] = { n->panning, 0.0f, 0.0f };
